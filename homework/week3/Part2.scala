...
abstract class Tree[T] {
  ...
  override def toString = traverse[String](
    proc_node = {(left:String, right:String, content:T) =>s"(${content.toString} ${left} ${right})"},
    proc_leaf ={content:T=> content.toString}
  )

  def leafs : List[T] = traverse[List[T]](
    proc_node = {(left:List[T], right:List[T], content:T) => left++right},
    proc_leaf ={content:T=> List(content)}
  )

  def evaluate : Int = traverse[Int](
    proc_node = {(left:Int, right:Int, content:T) =>
     content match {
       case "+" => left + right
       case "-" => left - right
       case "*" => left* right
       case _ => throw new ArithmeticException(s"Cannot handle operator $content")
     }
    ...
    proc_leaf ={content:T=>
      content match{
        ...
        case _ => throw new ArithmeticException(s"Cannot handle operand $content")
       }
      }
  )
}
